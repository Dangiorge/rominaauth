// path: app/api/inventory/items/[itemId]/route.js
...
export async function GET(req, { params }) {
  ...
  const item = await prisma.masterItem.findUnique({ where: { id: itemId } });
  if (!item)
...
export async function PUT(req, { params }) {
  ...
  const before = await prisma.masterItem.findUnique({ where: { id: itemId } });
  if (!before)
    return NextResponse.json({ error: "Item not found." }, { status: 404 });

  if (body.sku && body.sku !== before.sku) {
    const dup = await prisma.masterItem.findFirst({
      where: { sku: body.sku, NOT: { id: itemId } },
    });
    if (dup)
  ...
  if (body.barcode && body.barcode !== before.barcode) {
    const dup = await prisma.masterItem.findFirst({
      where: { barcode: body.barcode, NOT: { id: itemId } },
    });
    if (dup)
      return NextResponse.json(
  ...
  const trackExpiry = body.track_expiry ?? before.track_expiry;
...
export async function DELETE(req, { params }) {
  ...
  if (hard) {
    ...
    await prisma.masterItem.delete({ where: { id: itemId } });
  } else {
